...
class mec_env:
    def __init__(self,service_num,total_capacity,total_cycle,total_tasks,bandwidth,zipf):
        self.zipf=zipf
        ...
        self.o_shape=service_num*2+1
        #t时各agent动作
        self.action_shape=service_num*2
        #记录环境当前状态
        self.o=np.zeros(self.o_shape)
        #每个节点不同任务类型在单位时间内到达的数量
        self.arr_task_num=np.zeros(service_num)
        #边缘节点与云节点之间带宽MHz
        self.b_cloud=bandwidth
        #服务l所需的内存资源G
        self.service_m=2+np.random.rand(service_num)
        #服务l所需的CPU资源 GHz
        self.service_c=0.05+0.05*np.random.rand(service_num)
        #服务l的平均任务大小 MB
        self.arr_task_size=50+50*np.random.rand(service_num)
        #服务类型为l的任务传输至其他边缘服务器的延迟
        self.task_cloud_delay=np.zeros(service_num)
        #服务l的startup time
        self.startup=np.random.rand(service_num)*0.3+0.2
        #系统时隙间隔
        self.delta_t=1

    def get_task_num(self):
        zipf_para, service_num = self.zipf, self.service_num
        x = np.arange(1, service_num+1)
        y = zipfian.pmf(x, zipf_para, service_num)*self.total_tasks
